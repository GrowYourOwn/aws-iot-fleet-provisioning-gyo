...
class ProvisioningHandler:
    ...
    def core_connect(self):
        ...
        self.primary_MQTTClient = mqtt_connection_builder.mtls_from_path(
            ...
            cert_filepath="{}/{}".format(self.secure_cert_path,
                                         self.claim_cert),
            pri_key_filepath="{}/{}".format(self.secure_cert_path,
                                            self.secure_key),
        ...
        print("Connecting to {} with client ID '{}'...".format(
            self.iot_endpoint, self.unique_id))
    ...
    def on_connection_resumed(self, connection, return_code, session_present, **kwargs):
        print('connection resumed with return code {}, session present {}'.format(
            return_code, session_present))

    def get_current_certs(self):
        non_bootstrap_certs = glob.glob(
            '{}/[!boot]*.crt'.format(self.secure_cert_path))
        non_bootstrap_key = glob.glob(
            '{}/[!boot]*.key'.format(self.secure_cert_path))
    ...
    def enable_error_monitor(self):
        ...
        template_reject_topic = "$aws/provisioning-templates/{}/provision/json/rejected".format(
            self.template_name)
        certificate_reject_topic = "$aws/certificates/create/json/rejected"

        template_accepted_topic = "$aws/provisioning-templates/{}/provision/json/accepted".format(
            self.template_name)
        certificate_accepted_topic = "$aws/certificates/create/json/accepted"

        subscribe_topics = [template_reject_topic, certificate_reject_topic,
                            template_accepted_topic, certificate_accepted_topic]
        ...
        for mqtt_topic in subscribe_topics:
            ...
            print("Subscribed with {}".format(
                str(mqtt_topic_subscribe_result['qos'])))
    ...
    def register_thing(self, serial, token):
        ...
        self.primary_MQTTClient.publish(
            topic="$aws/provisioning-templates/{}/provision/json".format(
                self.template_name),
    ...
    def cert_validation_test(self):
        ...
        print("Connecting with Prod certs to {} with client ID '{}'...".format(
            self.iot_endpoint, self.unique_id + "-Prod"))
